...
class CveDetailsSpider(scrapy.Spider):
    ...
    def get_url(self, page, year, trc, sha):
        return "https://www.cvedetails.com/vulnerability-list.php?vendor_id=0&product_id=0&version_id=0&page={}&hasexp=0&opdos=0&opec=0&opov=0&opcsrf=0&opgpriv=0&opsqli=0&opxss=0&opdirt=0&opmemc=0&ophttprs=0&opbyp=0&opfileinc=0&opginf=0&cvssscoremin=0&cvssscoremax=0&year={}&month=0&cweid=0&order=1&trc={}&sha={}".format(
            page, year, trc, sha)
    ...
    def parse1(self, response):
        detailurls = response.selector.xpath(
            '//div[@id="searchresults"]/table/tr[@class="srrowns"]/td[@nowrap]/a/@href').getall()
    ...
    def parse2(self, response):
        ...
        cveid = response.selector.xpath('//h1/a/text()').extract_first()
        score = response.selector.xpath('//div[@class="cvssbox"]/text()').extract_first()
        ...
        for make in makes:
            ...
            item = CveItem()
            item['cveid'], item['score'], item['vulntype'], item['vendor'], item['product'], item['producttype'], item[
                'version'] = cveid, score, vulntype, vendor, product, producttype, version
            yield item